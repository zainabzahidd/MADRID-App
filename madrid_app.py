...
# ─── CRITERION SCALE BOUNDS ───────────────────────────────────────────────────
# Qualitative criteria use DIFFERENT scales. Do NOT clip them all to the same
# range — doing so silently corrupts legitimate values (e.g. C5=8, C6=9/10).
QUAL_BOUNDS = {
    'C5_Airline_Connectivity': (1, 8),
    'C6_Infrastructure_Quality': (1, 10),
    'C7_Strategic_Importance': (1, 10),
}
...
def load_data():
    ...
    # NOTE: RYK and CJL have C5 = 1 here, matching the decision matrix that feeds
    # the model. If you change these, update the Criteria Documentation file too.
    data = {
...
def run_madrid(df_input, expert_ranking, n_clusters_override=None):
    ...
    # ─── CRITERIA WEIGHTS ──────────────────────────────────────────────────────
    # NOTE ON METHOD LABELLING:
    # The line below computes weights with a LINEAR rank-sum formula
    # (w_i = (N - rank_i + 1) / sum). For the ranking used in this study this
    # yields 0.250, 0.214, 0.179, 0.143, 0.107, 0.071, 0.036 — the values
    # reported as the criteria weights. However, this is NOT the DIBR method:
    # true DIBR derives weights from pairwise ratios between adjacent ranked
    # criteria (Pamucar et al., 2021). Two options before final submission:
    #   (a) relabel this in the report/app as "rank-sum (rank-order centroid-style)
    #       weighting" so the description matches the code, OR
    #   (b) implement true DIBR and re-run (the weights — and therefore some
    #       lower-rank results — may change).
    # The computation is left unchanged here so all reported numbers stay identical.
...
if page == "Overview & Rankings":
    ...
    with col_right:
        ...
        display_df_show['Cluster'] = display_df_show['Cluster'].map({2:'★ Tier 1 (Hub)', 1:'Tier 2 (Regional)'})
...
# ═══════════════════════════════════════════════════════════════════════════════
    ...
    if step == "Step 1: Decision Matrix & Normalization":
        ...
        st.info(f"**2 clusters** were selected as optimal based on Silhouette Analysis (Score: {sil:.4f})")
        ...
        st.dataframe(scores_df.round(4), hide_index=True, use_container_width=True)
        ...
        st.dataframe(weight_df.round(4), hide_index=True, use_container_width=True)
...
# ═══════════════════════════════════════════════════════════════════════════════
    ...
    with col1:
        st.markdown("**C5: Airline Connectivity** (scale 1–8)")
        c5_delta = st.slider("Perturbation (±points)", -2, 2, 0, key="c5")
    with col2:
        st.markdown("**C6: Infrastructure Quality** (scale 1–10)")
        c6_delta = st.slider("Perturbation (±points)", -2, 2, 0, key="c6")
    with col3:
        st.markdown("**C7: Strategic Importance** (scale 1–10)")
        c7_delta = st.slider("Perturbation (±points)", -2, 2, 0, key="c7")

    # Run perturbed analysis — clip each criterion to ITS OWN scale bounds.
    df_perturbed = df_raw.copy()
    lo5, hi5 = QUAL_BOUNDS['C5_Airline_Connectivity']
    lo6, hi6 = QUAL_BOUNDS['C6_Infrastructure_Quality']
    lo7, hi7 = QUAL_BOUNDS['C7_Strategic_Importance']
    df_perturbed['C5_Airline_Connectivity'] = (df_perturbed['C5_Airline_Connectivity'] + c5_delta).clip(lo5, hi5)
    df_perturbed['C6_Infrastructure_Quality'] = (df_perturbed['C6_Infrastructure_Quality'] + c6_delta).clip(lo6, hi6)
    df_perturbed['C7_Strategic_Importance'] = (df_perturbed['C7_Strategic_Importance'] + c7_delta).clip(lo7, hi7)
...
# ═══════════════════════════════════════════════════════════════════════════════
    ...
    with col1:
        st.markdown("""
        ## What is MADRID?
        **MADRID** stands for *Multicriteria Analysis Directed by Ranking and Integrated
        Data-clustering*. It was proposed by **Corriça et al. (2025)** for unmanned aerial
        vehicle selection for the Brazilian Navy. This study **adapts** the framework to
        airport investment prioritization in Pakistan.

        The method combines three established techniques:
        - **Hierarchical Clustering** (Single Linkage) — groups alternatives by profile similarity
        - **DIBR** (Defining Interrelationships Between Ranked Criteria) — derives weights from ordinal criteria rankings
        ...
        | Dimensionality | Compares all at once | Clustering reduces the decision space |
    ...
    with col2:
        ...
        **Quantitative (from PCAA data):**
        ...
        - **C2** Passenger Volume *(Weight: 25.0%)*
        - **C3** Cargo Volume
        - **C4** Mail Volume

        **Qualitative (scored from documented rubric):**
        - **C5** Airline Connectivity *(scale 1–8)*
        - **C6** Infrastructure Quality *(scale 1–10)*
        - **C7** Strategic Importance *(scale 1–10; Weight: 21.4%)*
        ...
        st.markdown("""
        ## Reference
        Corriça, J. V. P., dos Santos, M., Pellanda, P. C., Goldschmidt, R. R.,
        de Araújo Costa, I. P., Moreira, M. Â. L., Martins, E. R., & Monteiro, F. M. B.
        (2025). Selection of an unmanned aerial vehicle for the Brazilian Navy: Novel
        approach for decision support through unsupervised machine learning.
        *Journal of Defense Analytics and Logistics, 9*(1), 2–25.
        """)
